...
public abstract class CCTestCase extends TestCase {
	...
	public static Collection<?> solutions() {
		...
		Supplier<IbexOptions> options2 = org.emoflon.ibex.tgg.run.cheat2facebooktoinstagram._RegistrationHelper::createIbexOptions;
		BiConsumer<ResourceSet, OperationalStrategy> reg2 = (t, u) -> {
			try {
				org.emoflon.ibex.tgg.run.cheat2facebooktoinstagram._RegistrationHelper.registerMetamodels(t, u);
			} catch (IOException e) {
				e.printStackTrace();
			}
		};
		
		Supplier<IbexOptions> options3 = org.emoflon.ibex.tgg.run.facebooktoinstagram._RegistrationHelper::createIbexOptions;
		BiConsumer<ResourceSet, OperationalStrategy> reg3 = (t, u) -> {
			try {
		...
		return Arrays.asList(new Object[][] { 
			...
			{ options2, reg2, "Cheat 2"},
			{ options3, reg3, "Your Solution"}
		});
	}
